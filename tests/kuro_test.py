...
def numeral_value(tok: Dict[str, Any], use_fractions: bool = USE_FRAC) -> Optional[float]:
    lab = tok.get('label_repo')
    if lab is not None and isinstance(lab, str) and lab.isdigit():
        return float(lab)
...
def ids_of(tok: Dict[str, Any]) -> Tuple[str, ...]:
    return tuple(i for i in tok['sign_ids'] if i)


def is_total(tok: Dict[str, Any], target: Optional[Tuple[str, ...]] = None) -> bool:
...
def is_commodity_heading(toks: List[Dict[str, Any]], j: int) -> bool:
...
def section_start(toks: List[Dict[str, Any]], idx: int, use_commodity: bool = True) -> int:
...
def cases(recs: Dict[str, Dict[str, Any]], target: Tuple[str, ...], use_commodity: bool = True) -> List[Tuple[str, List[float], float, int, int]]:
...
def is_damaged(tok: Dict[str, Any]) -> bool:
...
def mismatch_damage_breakdown(recs: Dict[str, Dict[str, Any]], target: Tuple[str, ...], use_commodity: bool = True) -> Dict[str, List[str]]:
    ...
    for rid, r in recs.items():
        ...
        for idx, t in enumerate(toks):
            ...
            after_tok: Optional[Dict[str, Any]] = None
            for u in toks[idx + 1:]:
                v = numeral_value(u)
                if v is not None:
                    after, after_tok = v, u
            ...
            start = section_start(toks, idx, use_commodity)
            # Store token objects, not just values
            section_toks: List[Dict[str, Any]] = [u for u in toks[start:idx] if numeral_value(u) is not None]
            vals = [numeral_value(u) for u in section_toks]
            vals = [v for v in vals if v is not None]
            if not vals or classify(vals, after) != 'mismatch':
                continue
            any_damage = is_damaged(after_tok) or any(is_damaged(u) for u in section_toks)
            (damaged if any_damage else clean).append(rid)
    return {'damaged': damaged, 'clean': clean}


def classify(vals: List[float], stated: float) -> str:
    s = float(sum(vals))
...
def null_rate(observed: List[Tuple[str, List[float], float, int, int]], trials: int = TRIALS) -> Tuple[float, float]:
...
def report(recs: Dict[str, Dict[str, Any]], target: Tuple[str, ...], name: str, use_commodity: bool = True) -> Optional[Tuple[List[Tuple[str, List[float], float, int, int]], collections.Counter[str], int]]:
